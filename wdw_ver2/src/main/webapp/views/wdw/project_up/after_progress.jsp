...
<c:set var="progress" value="${data.after_progress }"/>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
                        ...
                        <div class="funding_preparation_wrapper">
                        	<div class="inquiry_answer_wrapper">
                        		...
                        		<div >
									...
									<table class="table">
										<tbody>
											<c:forEach items="${progress }" var="pp" varStatus="no">
												<tr>
													<th scope="row">${no.index + 1 }</th>
													<td>
														<fmt:formatDate value="${pp.project_board_regdate }" pattern="yyyy-MM-dd " />
													</td>
													<td>${pp.project_board_title }</td>
													<td>${pp.getContentSub() }</td>
													<td>
														<input type="button" class="reply" value="상세" onclick="location.href='after_progressDetail?project_board_no=${pp.project_board_no}&pro_code=${pp.project_board_pro_no }'"/>
								...
								<div>
									<button type="button" class="writ_button" onclick="location.href='after_progressInsert?pro_code=${data.pro_code}'" >글쓰기</button>
								</div>
                        	</div>   
                        </div>
                    </div>
